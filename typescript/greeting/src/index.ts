...
interface SalesFiguresParameters {
  industry?: string;
  count?: number;
  year?: number;
}
 ...
 */
async function salesFigures(parameters: SalesFiguresParameters) {
  ...
  // Select industry
  const selectedIndustry = industry && industries[industry as keyof typeof industries] 
    ? industry 
    : Object.keys(industries)[Math.floor(Math.random() * Object.keys(industries).length)];
  
  const industryData = industries[selectedIndustry as keyof typeof industries];
